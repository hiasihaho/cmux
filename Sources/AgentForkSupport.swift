...
enum AgentForkSupport {
    ...
    private static func supportsLocalForkProbe(
        ...
        if usesDefaultDirectoryForMissingWorkingDirectory,
           forkProbeResolutionDependsOnWorkingDirectory(probe: probe, processEnvironment: processEnvironment) {
            return false
        }
    ...
    static func forkValidationExecutableResolution(
        ...
        let usesDefaultDirectoryForMissingWorkingDirectory = useDefaultDirectoryWhenWorkingDirectoryIsMissing
            && requestedWorkingDirectory.flatMap({ localDirectoryURL(path: $0) }) == nil
        let workingDirectory = useDefaultDirectoryWhenWorkingDirectoryIsMissing
            && requestedWorkingDirectory.flatMap({ localDirectoryURL(path: $0) }) == nil
            ? nil
            : requestedWorkingDirectory
        if usesDefaultDirectoryForMissingWorkingDirectory,
    ...
    private static func forkProbeResolutionDependsOnWorkingDirectory(
        probe: (executable: String, arguments: [String]),
        processEnvironment: [String: String]
    ) -> Bool {
        if probe.executable.contains("/") {
            return !probe.executable.hasPrefix("/")
        }
        return (processEnvironment["PATH"] ?? "")
            .split(separator: ":", omittingEmptySubsequences: false)
            .contains { directory in
                directory.isEmpty || !directory.hasPrefix("/")
            }
    }
