...
| Gap | Notes | Effort |
|---|---|---|
| Agent session auto-resume on restore | macOS: `terminal.autoResumeAgentSessions` (default true) — on session restore, each surface runs its agent's native resume command with the hook-recorded session id (`~/.cmuxterm/<agent>-hook-sessions.json`; 17-agent matrix incl. `claude --resume <id>`, `codex resume <id>`, Kimi Code via #7201 — kb/session-restore-and-resume.md). The RECORDING half is shared CLI and already runs on Linux; the restore half is unported — after every promote the human types `claude --continue` by hand. Wiring: match restored surfaces to session records, one-shot send-on-ready of the resume command, honor the setting. Would make promotes near-seamless | promote workflow + user ask 2026-08-18 | M |
