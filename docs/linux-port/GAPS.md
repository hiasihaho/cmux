...
| `feed.*` verbs unimplemented — agent-hook event pipeline dead-ends | `cmux rpc feed.list` / `feed.push` (opencode plugin, `hooks feed`) all unknown_method on Linux; the lfm-dl multi-agent event tap is parked on exactly this. Engine exists in shared CMUXAgentLaunch (WorkstreamStore/Event/Item + JSONL persistence) — wire it into CmuxAdw + mirror FeedSocketEncoding. Verifier: `~/olmo/exp/run feedprobe --watch` | roadmap/08 item 1 (lfm-dl, 2026-08-18) | M |
