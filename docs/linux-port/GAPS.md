...
| Gap | Notes | Effort |
|---|---|---|
| Workspace groups last mile: colors/icons + management UI | Verbs+model+persistence (stage 1) and sidebar sections with chevron/counts/attention aggregation (stage 2) landed 2026-07-24 (`workspace-groups-smoke`, 39 assertions incl. rendered-row assertions via `debug.sidebar_rows`). Remaining: render `custom_color`/`icon_symbol` on headers, and a UI affordance for group create/rename/move (context menu — the sidebar has none yet for workspaces either) | S |
