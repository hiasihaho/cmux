...
| Ghostty shim increment 4: port the embed layer to manaflow's expanded embedded API | The fork catch-up merge (16,853 commits, trial branch `trial-merge-probe` in the 2026-07-24 session worktree — merge + 4 fix commits done, xcframework proven on the VM) leaves `lib-gtk` failing with 20+ errors in manaflow's grown `embedded.zig` (`apprt.gtk.App` vs `apprt.embedded.App` at :213, `.userdata` missing on gtk Surface at :303 …). Until this lands: fork unpushed, parent submodule pointer unmoved, Linux keeps building from the pre-merge checkout. Payoff: the 15 new embedded APIs (read_screen_tail_vt, render_grid_json_with_theme, pty_tee …) become available to the Linux port too | POC-0003 increment 1, 2026-07-24 | M |
