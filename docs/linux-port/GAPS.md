...
| `notification.create_for_caller` ignores macOS's caller-target resolution | `cmux notify --workspace <other-ws>` from inside a pane sends `preferred_workspace_id` = the target AND `preferred_surface_id` = the CALLER's own surface (shared `CLI/cmux.swift`). macOS then applies issue #7939: a surface that is not in the preferred workspace OUTRANKS the workspace claim, so the notification follows the surface home — i.e. the same command notifies the sender's own workspace on macOS and the addressed workspace here. Our stored entry also carries a surface id from another workspace (ignored by open/jump, which check `contains(surfaceId:)`). Needs a DECISION, not just code: mirror #7939 exactly (accepting that an explicit `--workspace` gets overridden), or validate membership and drop the foreign surface — and upstream the observation that the CLI conflates "the workspace I was spawned in" with "the workspace the user typed" in one param | cross-session notify from the passkey desk, 2026-09-01 | S |
