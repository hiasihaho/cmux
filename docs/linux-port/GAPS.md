...
| Per-session scrollback directory | `ScrollbackStore.directory` is `dirname(sessionPath)/scrollback`, so every instance whose session lives in the same directory (all test suites, `/tmp`-session dev slots) SHARES it — and `prune(keeping:)` runs on every save, deleting the other instances' capture files. Cure: derive the dir from the session file name (`<session>-scrollback/`). Found while chasing the 2026-07-22 gate flakes; suites now defend with epoch-stamped verification, but the product behavior is the bug | harness batch flake hunt | S |
