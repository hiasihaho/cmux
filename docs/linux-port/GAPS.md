...
| **Claude Code Teams on Linux** — verify-first | `cmux claude-teams` + the tmux shim shipped with the merge and run on Linux. kb/tmux-compat.md verified the full dispatch in `CLI/cmux.swift`: ~25 tmux verbs handled, and **every socket verb they call is ✅ on the port**. Unverified: shim-script writing under `~/.cmuxterm/`, auto-layout behavior, macOS-pathed assumptions. One dogfood run of `cmux claude-teams` in a scratch workspace settles it — teammate agents as native splits would be a headline parity win | docs crawl + kb 2026-07-22 | S–M |
