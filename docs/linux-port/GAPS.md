...
| **Claude Code Teams on Linux** — verify-first | `cmux claude-teams` + the tmux shim (`CLI/CMUXCLI+TmuxCompatSupport.swift`) shipped with the merge and run on Linux; the shim's vocabulary (new-session/split-window/send-keys/capture-pane/select/kill/list) maps onto verbs the port implements. End-to-end test in a scratch workspace, fix whatever verb the shim finds missing — teammate agents as native splits would be a headline parity win | docs crawl + CLI check 2026-07-22 | S–M |
