...
| Gap | Notes | Effort |
|---|---|---|
| Passkeys P1b–P3: vault encryption-at-rest, verbs, credentialsd, CXF | P1a landed 2026-09-01 ([PASSKEYS.md](PASSKEYS.md)): client layer + ES256 software authenticator behind `CMUX_WEBAUTHN=1`, webauthn.io verified live, `webauthn-smoke` 10 + `webauthn-live` proof script. Remaining, in order: vault encryption via Secret Service (`org.freedesktop.secrets` — the port has NO secret-store abstraction yet, vault is 0600 plaintext JSON like the CLI's socket password); `cmux browser webauthn` verb group (list/rm credentials, dev virtual-authenticator story); credentialsd D-Bus backend for real USB keys/hybrid + ceremony picker; account picker for multi-credential RPs; CXF export/import, then the qmp P2P-sync experiment. Flag flip to a setting after dogfood | S–M each |
