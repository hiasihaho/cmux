...
| Linux feed persistence never writes its JSONL | `<session-stem>-feed.jsonl` is absent after 21 live ingests: `WorkstreamStore.ingest` appends via a fire-and-forget `Task {}` that inherits MainActor, and cmux-adw's GLib loop appears not to drain the MainActor executor — so persistence (and likely `store.start()` history load) silently never run on Linux. Feed is memory-only (ring 2000, dies at restart). Fix: synchronous append or a Linux-side executor pump; then decide deliberately about opt-in unredacted persistence (upstream redacts tool payloads by design, keeps prompts) — the lfm-dl "full-fidelity tap" ask hangs on that decision | feed-wiring session 2026-08-18 | S |
