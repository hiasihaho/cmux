...
| codex-teams / omc / omo / omx — real teammate splits | launcher+shim setup verified (teams-siblings-smoke, batch 1) — but the 6 honest skips are the *actual feature* (teammate-becomes-a-split), unverifiable here because the agent binaries are absent. **To close the skips, provision:** `codex` (Codex CLI — note codex-teams is app-server/watcher, NOT shim-based), `omc`/`omx` (`npm i -g oh-my-claude-sisyphus`), and the opencode oh-my-openagent plugin for `omo`. Then re-run teams-siblings-smoke with the split legs un-skipped. NB each launcher differs (wiring/02, kb/tmux-compat) | teams-siblings 2026-07-23 | S |
