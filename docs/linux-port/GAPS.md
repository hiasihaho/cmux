...
| Gap | Notes | Effort |
|---|---|---|
| Passkeys P1b–P3: vault encryption-at-rest, verbs, credentialsd, CXF | P1a landed 2026-09-01 ([PASSKEYS.md](PASSKEYS.md)): client layer + ES256 software authenticator behind `CMUX_WEBAUTHN=1`, webauthn.io verified live, `webauthn-smoke` 10 + `webauthn-live` proof script. Remaining, in order: **P1b vault encryption — build the vault-key provider with TWO backends** (host: gnome-keyring item via `org.freedesktop.secrets`; flatpak: the Secret **portal** `org.freedesktop.impl.portal.Secret`, which hands a per-app master secret with no keyring-wide access — preferred over a blunt `--talk-name=org.freedesktop.secrets` finish-arg; portal verified activatable on this host. The port has NO secret-store abstraction yet, vault is 0600 plaintext JSON like the CLI's socket password); `cmux browser webauthn` verb group (list/rm credentials, dev virtual-authenticator story); credentialsd D-Bus backend for real USB keys/hybrid + ceremony picker (a talk-name, not `--device=all`); account picker for multi-credential RPs; CXF export/import, then the qmp P2P-sync experiment. Flag flip to a setting after dogfood — which is also the flatpak answer: host env does not cross `flatpak run` (interim: `flatpak override --user --env=CMUX_WEBAUTHN=1`); a setting crosses, BUT today the sandboxed app reads the per-app `$XDG_CONFIG_HOME/cmux/cmux.json` (`~/.var/app/com.manaflow.cmux/config/…`), NOT the host `~/.config` file — `Settings.fileURL` prefers `XDG_CONFIG_HOME`, which flatpak always sets (measured in-sandbox by the cmux desk; see the "Flatpak reads a config file nobody edits" row in Now, whose resolution this flip depends on — until then host CLI and sandboxed app are split-brain on settings). The build itself needs no manifest change (swift-crypto already in the offline mirrors). Flatpak vault lands under `~/.var/app/…/data/cmux-flatpak/` — correctly separate from the host vault (session-dir isolation trap class, 2026-07-22); sharing would be a product decision, not an accident. `CMUX_WEBAUTHN_AUTOAPPROVE` must never appear in a manifest (consent dialog IS the security boundary) | S–M each |
