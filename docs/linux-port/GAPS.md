...
| Gap | Notes | Effort |
|---|---|---|
| Workspace groups: comfort remainder | Stages 1–3 landed 2026-07-24 (verbs, sidebar sections, colors/icons rendering, menu management; `workspace-groups-smoke` 47 assertions; adversarially QA'd — one hex-grammar defect found and fixed red-first). Remaining is the cross-cutting comfort family: drag-and-drop membership/reorder, right-click context menus, color-palette picker — being scoped by the macOS UX-comfort survey (UX-PARITY) rather than piecemeal here | M |
...
| UX-parity ❌ cluster | from [UX-PARITY.md](UX-PARITY.md) 2026-07-22, no decision needed (missing, not deviations): ~~unfocused-split dimming~~ (done 2026-07-23), ~~sidebar hover-close~~ (done 2026-07-24, mirror ②), GtkShortcutsWindow (Ctrl+?), notification card rows, surface-tab type icons, tab-bar end-action buttons, sidebar context menu (mirror ③, next). The comfort work now follows MACOS-UX.md's mirror order | S each, batchable |
