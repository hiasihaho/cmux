...
| Browser profiles (isolated cookie/storage/cache spaces) | ❌ | upstream macOS has `BrowserProfileStore` + `WKWebsiteDataStore(forIdentifier:)` + profile popover + `browser profiles` verbs (newer than our checkout — found during the 07-21 upstream survey). Linux design ready: roadmap/07-browser-profiles.md, maps to `WebKitNetworkSession`; parked until after the upstream catch-up merge |
