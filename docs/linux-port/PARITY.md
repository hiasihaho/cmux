...
| Terminal scrollback persistence | ✅ | the visible screen is stored (ANSI-safe truncation to a budget, as macOS does) and replayed on restore through the fork's `inject_output` message, so the text is parsed as terminal *output* and never reaches the shell as input. macOS replays via a temp file + environment variable + shell integration; the Linux path needs none of those |
