...
| Sibling team launchers (codex-teams / omc / omx / omo) | 🟡 | launcher + tmux-shim setup path verified on Linux (teams-siblings-smoke); real teammate splits unverified (agent binaries not installed here). Finding: omc/omx resolve their binary before writing the shim; codex-teams writes no tmux shim (app-server path). claude-teams itself is ✅ end-to-end |
