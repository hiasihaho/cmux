...
struct CMUXCLI {
    ...
    func run() throws {
        ...
        case "new-workspace":
            ...
            let background = rem1.contains("--background")
            let remaining = rem1.filter { $0 != "--background" }
            if let unknown = remaining.first(where: { $0.hasPrefix("--") }) {
                throw CLIError(message: "new-workspace: unknown flag '\(unknown)'. Known flags: --command <text>, --cwd <path>, --background")
            }
            var params: [String: Any] = [:]
            if let cwdOpt {
            ...
            if background {
                // Create without selecting — automation must not steal focus.
                params["focus"] = false
            }
    ...
    private func subcommandUsage(_ command: String) -> String? {
        ...
        case "new-workspace":
            return """
            Usage: cmux new-workspace [--cwd <path>] [--command <text>] [--background]
    ...
    private func usage() -> String {
        ...
        Commands:
          ...
          list-workspaces
          new-workspace [--cwd <path>] [--command <text>] [--background]
